#include 'cloneable_trigger_base.cpp'
...
class script {
	...
	void set_layer_properties(array<layer_properties> properties, int collision_layer) {
		set_layer_properties(properties, collision_layer, 0);
	}
...
class layer_control_trigger : cloneable_trigger_base {
	...
	[boolean] bool apply_in_editor = false;
	[text] int collision = DEFAULT_COLLISION;
	[text] array<layer_properties> layers;
	[slider,min:0,max:600] float transition_period = 0;
	...
	void trigger() {
		s.set_layer_properties(layers, collision, transition_period);
		s.apply_layer_changes();
	}
